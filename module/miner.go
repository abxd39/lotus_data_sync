...
func (mbig *BsonBigint) SetBSON(raw bson.Raw) error {
	...
	var isok bool
	if num == "" {
		utils.Log.Traceln("debug num 为空", num)
		num = "0"
	}

	if mbig.Int, isok = mbig.SetString(num, 10); !isok {
		utils.Log.Errorf("convert '%s' to bigint failed", num)
		return fmt.Errorf("convert '%s' to bigint failed", num)
	}
	//utils.Log.Traceln("debug", mbig.Int)
	return nil
}

func (m *BsonBigint) BigInt() big.Int {
	return *m.Int
}
...
type MinerStateAtTipset struct {
	...
	AccountBalance   float64 `bson:"account_balance" json:"account_balance"`
	AvailableBalance float64 `bson:"available_balance" json:"available_balance"`
	InitialPledge    float64 `bson:"initial_pledge" json:"initial_pledge"`
	LockedFunds      float64 `bson:"locked_funds" json:"locked_funds"`
...
type MinerInfo struct {
	...
	TotalPower        string  `bson:"total_power" json:"total_power"`
	WalletAddr        string  `bson:"wallet_addr" json:"wallet_addr"`
	SectorSize        uint64  `bson:"sector_size" json:"sector_size"`
	SectorCount       uint64  `bson:"sector_count" json:"sector_count"`
	BlockCountPercent float64 `bson:"block_count_percent" json:"block_count_percent"`
	PowerPercent      float64 `bson:"power_percent" json:"power_percent"`
	ProvingSectorSize string  `bson:"incoming_sectorsize" json:"incoming_sectorsize"`
	TipsetHeight      uint64  `bson:"tipset_height" json:"tipset_height"`
	MineTime          uint64  `bson:"mine_time" json:"mine_time"`
	AccountBalance    float64 `bson:"account_balance" json:"account_balance"`
	AvailableBalance  float64 `bson:"available_balance" json:"available_balance"`
...
func BulkUpsertMiners(miners []interface{}) error {
	//utils.Log.Traceln("BulkUpsertMiners")
...
func UpsertMinerStateInTipset(miner *MinerStateAtTipset) error {
	...
	_, err := Upsert(MinerCollection, bson.M{"miner_addr": miner.MinerAddr, "tipset_height": miner.TipsetHeight}, minerInfo)
	return err
}
