...
type FilscanMsg struct {
	...
	//MessageParams string           `json:"message_params" json:"message_params"`
...
type BalanceTransferResp struct {
	M struct {
		To           string `bson:"To" json:"to"`
		FromAddress  string `bson:"From" json:"from_address"`
		BalanceValue string `bson:"Value" json:"balance_value"`
		//MessageCid   string `bson:"CID:/" json:"message_cid"`
		BalanceType int    `bson:"Method" json:"balance_type"`
		GasFeeCap   uint64 `bson:"gas_fee_cap" json:"gas_fee_cap"`
	...
	CreateTime uint64 `bson:"msg_create" json:"create_time"`
}
...
func GetMsgByAddressFromMethodLimit(address, fromTo, methodName string, begindex, count int, beginTime, endTime int64) (res []*FilscanMsgResult, total int, err error) {
	if len(address) < 1 {
		return
	}
	q := bson.M{}
	...
	if beginTime != 0 && endTime != 0 {
		bm := []bson.M{}
		q["$and"] = append(bm, bson.M{"msg_create": bson.M{"$gte": beginTime, "$lte": endTime}})
	}
	if len(methodName) != 0 {
		q["method_name"] = methodName
	}
	sel := bson.M{"message.Params": 0}
	if count > 0 {
		utils.Log.Traceln(q)
		//err = FindAllLimit(MsgCollection, q, nil, &res, begindex, count)
		err = FindSortLimit(MsgCollection, "-msg_create", q, sel, &res, begindex, count)
		if err != nil {
...
func GetMsgCount(to string) (total int, err error) {
	q := bson.M{"message.To": to}
	return FindCount(MsgCollection, q, nil)
}

//miner msg get gas
func GetSumGasPriceByMsgMinCreate(MinTime int64) (sum float64, err error) {
	o1 := bson.M{"$group": bson.M{"_id": "", "totalGasPrice": bson.M{"$sum": bson.M{"$toDouble": "$message.GasPrice"}}}}
...
//msg size
func GetSumSizeByMsgMinCreate(MinTime int64) (sum uint64, err error) {
	o1 := bson.M{"$group": bson.M{"_id": "", "totalSize": bson.M{"$sum": "$size"}}}
	o0 := bson.M{"$match": bson.M{"msg_create": bson.M{"$gte": MinTime}}}
	operations := []bson.M{o0, o1}
	type result struct {
...
//msg count
func GetMsgCountByMsgMinCreate(MinTime int64) (total int, err error) {
...
// func GetMsgByBalanceTranceListLimit(input *filscanproto.BalanceTransferReq) (res []*filscanproto.BalanceTransferResp_Data, total int, err error) {
// 	q := bson.M{}
// 	List := make([]BalanceTransferResp, 0)
// 	//utils.Log.Traceln("param ---- address ", input.Address)
// 	if input.Address != "" { //获取该地址的所有信息
// 		q = bson.M{"message.To": input.Address}
// 	} else {
// 		//copy来了
// 		q["msg_create"] = bson.M{"$gt": TimeNow - 60*60*24*7}
// 	}
// 	if input.MethodName != "" {
// 		q["method_name"] = input.MethodName
// 	}
// 	if input.BeginTime != 0 && input.EndTime != 0 {
// 		bm := []bson.M{}
// 		q["$and"] = append(bm, bson.M{"msg_create": bson.M{"$gte": input.BeginTime, "$lte": input.EndTime}})
// 	}
// 	utils.Log.Tracef("%+v", q)
// 	if input.Count > 0 {
// 		err = FindSortLimit(MsgCollection, "-msg_create", q, nil, &List, int(input.Begindex), int(input.Count))
// 		if err != nil {
// 			return nil, 0, err
// 		}
// 	}

// 	//utils.Log.Tracef("%+v", List)
// 	total, err = FindCount(MsgCollection, q, nil)
...
// 	}
// 	for _, v := range List {
// 		r := &filscanproto.BalanceTransferResp_Data{}
// 		r.MessageCid = v.M.C.Cid
// 		r.FromAddress = v.M.FromAddress
// 		r.CreateTime = v.CreateTime
// 		balance, err := types.BigFromString(v.M.BalanceValue)
// 		if err != nil {
// 			utils.Log.Errorln(err)
// 			continue
// 		}
// 		r.BalanceValue = types.FIL(balance).String()
// 		r.To = v.M.To
// 		r.BalanceType = v.MethodName
// 		res = append(res, r)
// 	}
// 	return
// }
