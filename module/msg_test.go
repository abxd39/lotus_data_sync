...
func TestGetSumGasPriceByMsgMinCreat(t *testing.T) {
	sum, err := GetSumGasPriceByMsgMinCreate(1576496942)
	if err != nil {
		t.Errorf("GetSumGasPrice() error = %v ", err)
		return
	}
...
func TestGetMsgCountByMsgMinCreat(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			gotTotal, err := GetMsgCountByMsgMinCreate(tt.args.MinTime)
			if (err != nil) != tt.wantErr {
				t.Errorf("GetMsgCountByMsgMinCreate() error = %v, wantErr %v", err, tt.wantErr)
				return
			}
			if gotTotal != tt.wantTotal {
				t.Errorf("GetMsgCountByMsgMinCreate() gotTotal = %v, want %v", gotTotal, tt.wantTotal)
			}
		})
	}
	*/
	gotTotal, err := GetMsgCountByMsgMinCreate(1576578555)
	if err != nil {
		t.Errorf("GetSumGasPrice() error = %v ", err)
		return
	}
...
func TestGetSumSizeByMsgMinCreat(t *testing.T) {
	gotTotal, err := GetSumSizeByMsgMinCreate(1577697456)
	if err != nil {
		t.Errorf("GetSumGasPrice() error = %v ", err)
		return
	}
